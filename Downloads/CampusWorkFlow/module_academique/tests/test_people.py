def create_user(client, email="jean.dupont@example.com", role="Teacher"):
    resp = client.post(
        "/users/",
        json={"name": "Jean Dupont", "email": email, "role": role},
    )
...
def test_teacher_creation(client):
    ...
    resp = client.post(
        "/teachers/",
        json={"name": "Prof Test", "email": "prof@example.com", "user_id": user["user_id"]},
    )
    assert resp.status_code == 201
...
def test_student_full_flow(client):
    faculty_resp = client.post("/faculties/", json={"name": "Sciences"})
