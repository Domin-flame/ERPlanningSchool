...
def upsert_student_ref(student: StudentRef, session: Session = Depends(get_session)):
    """
...
def list_invoices_for_student(student_id: int, session: Session = Depends(get_session)):
...
def _confirm_payment_sync(reference: str):
    ...
    with Session(engine) as session:
        ...
        if not payment or payment.statut != "EN_ATTENTE":
            return
        _apply_confirmed_payment(session, payment)


@app.post("/payments/momo/confirm/{reference}", tags=["payments"])
def confirm_momo_payment_manual(reference: str, session: Session = Depends(get_session)):
    """
...
def _apply_confirmed_payment(session: Session, payment: Payment):
    ...
    total_paye = session.exec(
        select(Payment).where(Payment.id_invoice == invoice.id_invoice, Payment.statut == "CONFIRME")
    ).all()
...
def list_leads_for_campaign(campaign_id: int, session: Session = Depends(get_session)):
