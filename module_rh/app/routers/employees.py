...
def create_employee(
    ...
    user=Depends(require_roles(["Super Admin", "Admin", "Staff"])),
):
...
def get_employee(
    ...
    # Un employé peut voir sa propre fiche ; admin/hr voient tout le monde
...
def deactivate_employee(
    employee_id: str,
    db: Session = Depends(get_db),
    user=Depends(require_roles(["Super Admin", "Admin"])),
):
    employee = db.get(models.Employee, employee_id)
    if not employee:
        raise HTTPException(404, "Employé introuvable")
